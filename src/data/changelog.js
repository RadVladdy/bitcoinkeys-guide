...
export const changelog = [
  ...
  },
  {
    date: '2026-08-01',
    type: 'feature',
    title: 'New page: roll your own seed with dice — the real procedure, and it never asks what you rolled',
    detail: 'The advisory says that 50+ of your own dice rolls kept people out of scope, which is useful right up to the moment you try to actually do it. There is now a step-by-step procedure — Roll your own seed with dice, in the Hardware & services menu — with the current menu path for every device we rate that has one. THE FIRST THING IT SAYS IS THAT IT DOES NOT WANT YOUR ROLLS. There is no input on the page and there is not going to be one: the sequence of numbers you roll IS your seed in a different alphabet, and typing it into a web page is the same mistake as typing your seed words into one. The most the page does is count taps for you — a tally that never learns a value, held in memory, gone on reload. WHAT WE FOUND WHILE VERIFYING IT: there is no single Coldcard dice path — it depends on which one you own, and most guides on the internet give you one of them as though it were the answer. On a Q, Mk4 or Mk5 it is New Seed Words → Advanced → 12/24 Word Dice Roll. On the Mk3 — the model hit hardest by the entropy flaw, so the one most likely to be in your hand this week — Coinkite’s own current advisory sends you to Import Existing → Dice Rolls instead. Both are live menus on live firmware. Coldcard is also the only maker here where the device takes your rolls directly and hashes them. Everything else — Blockstream Jade, BitBox02, Passport Core — works the opposite way round: you pick 11 or 23 words from the vendor’s printed lookup table and the device only calculates the final checksum word. Jade’s own method needs two 16-sided dice and an 8-sided one, not a d6, which is worth knowing before you clear an evening. Trezor has no dice path at all and says in writing that you should never choose your own backup; Foundation’s Passport Prime documentation is silent on it. We say all of that on the page rather than papering over the gaps. Also on it: the arithmetic (one roll is log₂6 ≈ 2.585 bits, so 50 rolls clears 128 and 99 lands at 255.9 — roll 100 and stop thinking about it), what you need, the seven ways this goes wrong, and the fact that verifying the maths afterwards means typing your rolls into a computer, which Coinkite says to do only on an offline amnesic system and which you should otherwise skip. The interactive dice demo stays where it is and still rolls throwaway numbers for you; the two pages now point at each other and do different jobs.',
  },
  {
    date: '2026-08-01',
    type: 'security',
