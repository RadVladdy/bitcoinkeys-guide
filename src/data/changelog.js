...
export const changelog = [
  {
    ...
    detail: 'The home page has been reworked around two things it was not doing. First, the promises: this site sells no device, holds no keys, covers Bitcoin only, and — new, and now stated out loud — logs nothing. No analytics, no trackers, no cookies. Anything you save stays in your own browser unless you deliberately export it, and we never see it because there is nowhere for it to go. That set of promises is the reason this guide can say what it says about hardware, so it is now the second thing on the page rather than a grey strip most people scrolled past. It also used to be made three separate times in three separate places, each one quieter than the last; it is made once now, properly. Second, the page never showed you the site. There is now a plain map of what is here: the course, laid out as five levels with the optional "under the hood" level visibly set apart so you can see at a glance what you can skip; the setup finder and the checklist it produces; and the hardware comparison with its published standard alongside the collaborative-custody services. And the interactive demos, which are the most unusual thing on this site and were never mentioned on the front page at all, now get a row of cards with a small diagram of what each one actually does. Two sections went: "Two doors in" offered the same two choices the top of the page already offers, and "The skills, in plain English" listed a subset of lessons as though it were the course. Nothing was removed from the site itself — only from a page that was repeating itself.',
  },
  {
    date: '2026-07-30',
    type: 'feature',
    title: 'New: a short knowledge check at the end of two lessons',
    detail: 'Two lessons — "Not your keys, not your coins" and "Send Bitcoin safely" — now end with a couple of questions on what you just read. Get one wrong and you get a short explanation of why, written to teach the point rather than just mark you down; get it right and it simply moves on. This is a pilot on two lessons. If it earns its place it will spread across the course, one question set per lesson, matched to the rule that lesson exists to teach. Two things worth being clear about. It is a check of your understanding, not of your setup — the tool that recommends a setup is "Find your setup," which is a different thing entirely and asks about your situation rather than testing you. And nothing here is recorded: your answers are not scored against you, not added to your plan, not stored in your browser, and not sent anywhere. Close the tab and they are gone. That is deliberate — this site keeps nothing about you, and a practice question is not a reason to start.',
  },
  {
    date: '2026-07-30',
    type: 'content',
    title: 'The quiz is now "Find your setup" — and two safety sections got more specific',
    detail: 'The setup quiz has been renamed. It was never a quiz: a quiz tests what you already know, and this asks about your situation — how you hold today, what worries you, who else is involved, what hardware you already own — and hands back a setup, a roadmap from where you are, and a checklist. The page\u2019s own heading has said \u201cFind your setup\u201d since the day it launched; every button pointing at it said \u201ctake the quiz.\u201d They now agree, the menu it sits under is called \u201cYour setup\u201d rather than \u201cTake action,\u201d and its address moved from /quiz to /find-your-setup with the old one redirecting, so nothing you have bookmarked breaks. Nothing about the questions or the recommendations changed. Two lessons also got more specific where they had been vague. On privacy and physical safety, the advice to spread your keys across several places was true but incomplete, and the incompleteness mattered: if you hold a single set of recovery words, every place you keep a copy is a place that holds everything, so spreading copies around buys you protection from fire and from a search, but it cannot make any one location \u201cnot enough\u201d to take your coins. What actually does that is how your keys are arranged — a passphrase stored apart from the words, a 2-of-3 multisig with keys in different places, or a collaborative setup where a service holds one of three — and the page now names each of them instead of gesturing at \u201cit depends on your setup.\u201d And the inheritance lesson now says plainly what it had left implied: handing someone your keys does not make the Bitcoin legally theirs. Bitcoin has no concept of a will, an heir or an estate, so somebody can hold your keys without being entitled to the coins, and somebody can be entitled to them and unable to touch one. Nothing on this site is a will, a trust or a beneficiary designation, none of it transfers legal title, and none of it is legal advice — it is an access plan, and it needs a legal document built beside it.',
  },
  {
    date: '2026-07-30',
    type: 'device',
    title: 'Correction: two Trezors were listed as clearing the bar with no caveat, and they have two',
    detail: 'The page explaining how we rate hardware said the Trezor Safe 3 and Trezor Safe 5 “clear the bar with no caveat at all.” They do clear it — nothing about their tier has changed, and we would still put savings on either — but calling them caveat-free was wrong, and the comparison page had been saying so all along. Both carry an amber mark there on Bitcoin-only and on portable recovery, and both marks are about something you should know before you buy. They ship multi-coin by default, so the Bitcoin-only firmware is a build you have to go and choose; and their Shamir backup, which the Safe 5 uses by default, splits your recovery into shares that only restore in wallets that understand that format — take the plain 12 or 24 words instead if you want a backup that restores anywhere. The Safe 7 already carried a caveat about being multi-coin, which is a trait all three share, so the same reasoning simply had not been applied to its two siblings; its note now mentions the backup format too. That leaves the BitBox02 as the only device in the cold-storage tier with no caveat against it. In the same pass, the standard’s three tiers were reordered so the tier we actually recommend is explained first rather than last — the summary cards at the top of the page had always run best first, while the detail below ran worst first. The Coldcard Mk5 also stopped answering the “tight budget” filter: at its price it sits above every budget the filter offers.',
  },
  {
    date: '2026-07-30',
    type: 'content',
    title: 'Two new lessons: where your money lives, and the two options that aren’t rungs',
    detail: 'The ladder lesson was carrying two things that were not the ladder. The first was a section on how much of your Bitcoin belongs at each “temperature” — hot meaning a wallet whose keys are reachable from the internet, cold meaning one that is not. That is a question about your money, not about how your keys are arranged, and it now has its own lesson at the start of the level, where it belongs: you decide which money you are protecting before you decide how to protect it. It is joined there by the explanation of what hot and cold actually mean, which had been sitting a level earlier in the lesson on keys — the definition and the decision it exists for are one subject and are now one page. The second was a pair of callouts about BIP-85 and Shamir backup. Neither is a rung — neither changes how many keys it takes to sign — and a two-sentence callout could not do either of them justice. They now share a lesson of their own, BIP-85 first (it makes your backups fewer, by deriving many seeds from one master) and Shamir second (it makes one backup split, into shares where any few rebuild it and one alone reveals nothing), with what each costs you stated plainly and a link to the interactive demonstration of each. It sits before the lesson on choosing a hardware wallet on purpose, because Shamir needs device support and that is worth knowing before you spend money. The ladder page itself is now only the ladder: a comparison table of all four rungs at the top, then each rung in full. The row of rung tiles that used to sit above the table is gone — it was showing you the same rungs twice.',
  },
  {
    date: '2026-07-30',
    type: 'content',
    title: 'The configuration ladder is now one lesson instead of five pages',
    detail: 'The ladder used to be an overview page plus a separate page for each of the four rungs. Reading it meant leaving the course, going out to a rung, and finding your own way back — and those rung pages were not part of the numbered course at all, so they had no “next lesson” to return you to it. They are now sections of the ladder lesson itself, in order, with a comparison table at the top that puts all four side by side: what each rung is best for, what it costs, what it solves, and what it costs you. Every old address still works and lands on the right section. Nothing about the rungs themselves changed — there are still four, and the advice is the same. What did change is what came out. Each rung page carried its own paragraph on inheritance, which the inheritance lesson already covers for all four setups in one place, so that is gone from here. The step-by-step “how to set it up” walkthrough is gone too: this side of the site is for learning, and the checklist is where the doing lives — and it gained the one step that walkthrough had which the checklist did not, setting a PIN on your device. The “save this setup to my plan” buttons are gone from the lesson pages as well; saving belongs with the quiz and your plan, not in the middle of a lesson. Finally, Shamir backup and BIP-85 moved out to the “under the hood” level, where their interactive demonstrations already were. Neither is a rung — they are optional refinements you can add at any rung — and having the explanation in one place and the demonstration in another helped nobody.',
  },
  {
    date: '2026-07-30',
    type: 'content',
    title: 'Fixed: some “how →” links on the quiz and checklist pointed at the wrong lesson',
    detail: 'When the quiz builds your checklist, each step carries a “how →” link to the lesson that explains it. Sixteen of those pointed at the wrong lesson. Every one of them opened a real page, so nothing looked broken — you simply arrived somewhere that did not answer the step you had clicked. They dated from July’s restructure, when lessons were merged, renamed and split in two, and were never re-read against the new shape. The worst were the inheritance ones: that material split into “why Bitcoin doesn’t inherit like money” and “build the Recovery Kit,” and every step telling you to write or review the Kit still pointed at the half that only describes the problem. Others sent you to the page about choosing a hardware wallet when the step was about picking a phone wallet or a collaborative-custody service, and two steps about backing up your wallet descriptor pointed at a page that never mentions descriptors. All of them now point where they say they do. One step lost its link and gained a fuller explanation instead: Bitkey’s recovery works unlike anything else in this guide — there is no seed phrase to write down — so no lesson here genuinely applied, and sending you to one would have been worse than sending you nowhere. And one piece of the quiz’s own reasoning was arguing with itself: if you said your worry was losing access yourself, or that you were not sure, the result would explain — correctly — why it was deliberately not giving you a passphrase, and then offer a passphrase as your next step. A passphrase defends against someone finding your recovery words; it does nothing about losing access yourself, and it adds one more secret you can lose. The step-up in that case is now 2-of-3 multisig, where any two of three keys can recover you, so no single lost key can strand you — the answer to the worry you actually named. Finally, the quiz gained a fifth thing you can say you are worried about: an exchange going bust, freezing your account, or getting hacked. That is the risk most people arrive here with, and until now there was no way to name it.',
  },
  {
    date: '2026-07-30',
    type: 'security',
